...
# 取得期間。"1mo" にして上位足(4H)の本数を確保し、ライブのダウ判定を安定させる
# （5d だと4H足が約30本でUNKNOWN多め）。SMA/ATR/STALE は末尾のみ参照のため影響なし。
# yfinance の5分足は約60日まで取得可能なので "1mo" で問題ない。
PERIOD = "1mo"            # 取得期間
# SMA本数。だまし(whipsaw)低減のため 20/50 → 50/100 に変更。
# 3年・5分足・3ペア実測: 20/50 はだまし(3h未満で反転)が516回、50/100 では23回(95%減)で
# gross もプラス維持。反応はやや遅く、シグナル頻度は約3.65→2.3回/日に減る。
SHORT_SMA = 50            # 短期SMAの本数
